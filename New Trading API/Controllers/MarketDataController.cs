        ...
        public HttpResponseMessage GetAheadRTDDesc(int SiteID, int UnitID)
        {
            ...
            using (TradingEntities db = new TradingEntities())
            {
                ...
                if(unit.TypeID == "BATT" || unit.TypeID == "BAT")
                {
                    ...
                    var ptRDValue10 = ptRDPlus10.RecordedValue(new AFTime(DateTime.Now.AddMinutes(minMod + 5).ToShortDateString() + " " + DateTime.Now.AddMinutes(minMod + 5).ToString("HH:mm:00")), AFRetrievalMode.Exact);
